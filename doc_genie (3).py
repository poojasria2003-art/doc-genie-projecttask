...
Original file is located at
    https://colab.research.google.com/drive/1vEDOQ94WP4v-7hmcgmcpfbY1Pg_HcZCm
"""
...
from reportlab.platypus import SimpleDocTemplate, Paragraph, Spacer, PageBreak, Table, Preformatted
...
class DocGenieAnalyzer:
    ...
    def extract_function_signature(code):
        ...
        try:
            ...
            # print(f"Error in extract_function_signature: {e}") # For debugging
            return None, None
        ...
        try:
            ...
            for node in ast.walk(func_def):
                if isinstance(node, ast.For):
                    ...
                    if node.value:
                        analysis['return_types'].append(ast.unparse(node.value))
            ...
            if len(analysis['description']) < 20:
                analysis['description'] = "Performs computation based on input parameter"

            return analysis
        except Exception as e:
            # print(f"Error in analyze_function_logic: {e}") # For debugging
            return {
                'has_loops': False,
                'has_conditions': False,
                'has_return': False,
    ...
    def generate_google_docstring(signature, analysis, code):
        ...
        if signature['params']:
            ...
            for param in signature['params']:
                ...
                docstring += f"    {param['name']} ({param['type']}): Parameter for{default_str}\n"

        docstring += f"\n Returns:\n    {signature['return_type']}: The result of\n"
        docstring += '    """\n'
    ...
    def generate_numpy_docstring(signature, analysis, code):
        ...
        if signature['params']:
            ...
            for param in signature['params']:
                ...
                docstring += f"    Parameter for controlling {param['name']} behavio\n"
...
analyzer = DocGenieAnalyzer() # Corrected class instantiation
generation_history = []
...
def generate_docstring(code, style="google"):
    ...
    try:
        ...
        if style.lower() == "google": # Indentation fix
...
def download_pdf():
    ...
    if not generation_history:
        return "X No documentation generated!", None
    ...
    try:
        ...
        story.append(Paragraph(f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", styles['Normal'])) # Added styles['Normal']
        ...
        story.append(Spacer(1, 0.3*inch))
        story.append(Paragraph(f"Total Documentations: {len(generation_history)}", styles['Normal'])) # Added styles['Normal']
...
def generate_facebook_link():
    ...
    message = f"I generated {len(generation_history)} professional Python function docst"
    encoded = urllib.parse.quote(message)
...
with gr.Blocks(
    ...
    code-block { background: #0f3460; border: 2px solid #4ECDC4; border-radius: 8px; padding: 10px; }
    """) as demo:
    gr.Markdown(
    """
    # 📚 Doc-Genie v2.0
    ...
    with gr.Row():
        with gr.Column(scale=3):
            # Code Input Area
            gr.Markdown("### Input Function Code") # Removed extra quotation marks
            ...
            # File Upload
            gr.Markdown("### File Upload") # Added label for the section
            file_upload = gr.File(label="Upload Python File") # Uncommented file_upload
            ...
            file_upload.change(process_and_display, inputs=[file_upload], outputs=[code_input, upload_status]) # Changed output to code_input, added upload_status

            # Style Selection
            with gr.Row():
                docstring_style = gr.Radio(["google", "numpy"], value="google", label="Docstring Style")
                generate_btn = gr.Button("✨ Generate Docstring", size="lg", variant="primary")
        ...
        with gr.Column(scale=1):
            gr.Markdown("## ⚙️ Controls") # Removed extra quotation marks

            clear_btn = gr.Button("🗑️ Clear History", variant="stop", size="sm")
            clear_btn.click(clear_history, outputs=[upload_status]) # Changed output to upload_status for consistency

            gr.Markdown("----")
            gr.Markdown("## 📦 Export") # Removed extra quotation marks
            ...
            gr.Markdown("----")
            gr.Markdown("### 📁 Share") # Removed extra quotation marks
            ...
            gr.Markdown("----")
            gr.Markdown("### ✨ Features") # Removed extra quotation marks
            gr.Markdown(
                """
**Improvements:**
 - ✔️ Accurate logic analysis
 - ✔️ Contextual descriptions
 - ✔️ Operation detection
 - ✔️ Loop & condition recognition
 - ✔️ Better PDF export
 - ✔️ Consistent output
